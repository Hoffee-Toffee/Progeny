...
function isValidValue(
  ...
  if (expectedType === 'number') {
    ...
    if (
      ...
      // Ensure blockName exists and pass context
  ...
  if (expectedType === 'variable') {
    // This is for validating inputs to 'get' or 'set's var field
...
function isValidBlock(block, inputVariables = [], consoleLog = false) {
  ...
  if (block.blockName === 'if' || block.blockName === 'ifElse') {
    ...
    if (
      ...
      block.actions.some((a) => !isValidBlock(a, inputVariables, consoleLog))
    ) {
      warn(
        `Invalid block: ${
          block.blockName
    ...
    if (
      ...
      (!Array.isArray(block.elseActions) ||
        block.elseActions.some(
          (a) => !isValidBlock(a, inputVariables, consoleLog)
        ))
    ) {
      warn(
...
async function generateInput(
  ...
  // Specific logic for type (if not at max depth)
    ...
    if (Math.random() < 0.1) {
      // Lower chance for boolean blocks for now
...
export class ProgenyProgram {
  ...
  async executeBlock(block, runTimeInputs, state) {
    ...
    if (blockName === 'set') {
      ...
      state.vars[block.var] = await this.resolveInput(
        block.value,
        runTimeInputs,
        state,
        varType
    ...
    let resolvedInputs = []
    if (blockDef.inputs) {
      // Only try to resolve inputs if blockDef defines them
      if (Array.isArray(block.inputs)) {
        // And if the block instance has an inputs array
  ...
  static async generateRandomBlock(
    ...
    const blockTypes = isAction
      ? Object.keys(blocksModule).filter(
          (t) => !blocksModule[t].output && t !== 'return' && t !== 'set'
        )
      : Object.keys(blocksModule).filter(
          (t) =>
            blocksModule[t].output === 'number' && t !== 'get' && t !== 'return'
        )
...
function optimizeDeadStores(blocks, inputVariables, consoleLog = false) {
  ...
  for (let i = blocks.length - 1; i >= 0; i--) {
    ...
    if (currentBlock.blockName === 'set') {
      ...
      // Other block types (add, get as statement, etc.)
      isBlockKept = true
      blockSpecificLiveVars = getReferencedVariables(
        currentBlock,
        inputVariables,
